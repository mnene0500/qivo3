...
function BlockedUserItem({ userId, onUnblock }: { userId: string, onUnblock: (id: string) => void }) {
  ...
  if (loading) return <div className="flex items-center gap-4 px-6 py-4 animate-pulse"><div className="w-12 h-12 rounded-full bg-gray-100" /><div className="flex-1 space-y-2"><div className="h-4 w-24 bg-gray-100 rounded" /></div></div>
...
export default function BlockedListPage() {
  ...
  useEffect(() => {
    if (!user?.id) return
    supabase.from('users').select('blocking').eq('uid', user.id).single().then(({ data }) => {
      setBlockedUids(data?.blocking || [])
      setLoading(false)
    })
  }, [user?.id])
