...
export function CallManager() {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`calls:${user.id}`)
      .on('broadcast', { event: 'incoming-call' }, (payload) => {
        ...
        setTimeout(() => { if (incomingCall) handleReject(); }, 45000)
      })
  ...
  const handleReject = async () => {
    ...
    setIncomingCall(null)
    supabase.channel(`calls:${callData.callerId}`).send({ type: 'broadcast', event: 'call-rejected' })
    // Log rejection in chat
    await supabase.from('messages').insert({ chat_id: callData.chatId, text: "[Rejected]", sender_id: user!.id, timestamp: Date.now() })
  }
      ...
      <div className="relative flex flex-col items-center gap-10 text-center">
        <div className="relative">
          ...
          <Avatar className="w-44 h-44 border-4 border-[#00A2FF] shadow-2xl relative z-10">
            ...
            <AvatarFallback className="bg-gray-800 text-white"><User className="w-16 h-16" /></AvatarFallback>
          </Avatar>
        </div>
        <div className="space-y-2">
          <h2 className="text-4xl font-black text-white tracking-tight">{incomingCall.callerName}</h2>
          <p className="text-[#00A2FF] font-black uppercase tracking-[0.3em] text-xs animate-pulse">Incoming {incomingCall.type} call...</p>
        </div>
        <div className="flex gap-14 mt-6">
          <button onClick={handleReject} className="w-20 h-20 rounded-full bg-red-500 flex items-center justify-center shadow-[0_0_50px_rgba(239,68,68,0.4)] transition-all active:scale-90"><PhoneOff className="text-white w-8 h-8" /></button>
          <button onClick={handleAccept} className="w-20 h-20 rounded-full bg-green-500 flex items-center justify-center shadow-[0_0_50px_rgba(34,197,94,0.4)] animate-bounce transition-all active:scale-90">
