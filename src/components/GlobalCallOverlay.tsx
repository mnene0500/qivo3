...
/**
 * @fileOverview Global Ringing Listener and Active Call UI with ringtone integration.
 */
export function GlobalCallOverlay() {
  ...
  // MANAGE RINGTONE PLAYBACK BASED ON CALL STATUS
  useEffect(() => {
    if (incomingCall) {
      ringtoneRef.current?.play().catch(() => {
        console.warn("Incoming call ringtone blocked by browser policy.");
      });
    } else {
      ringtoneRef.current?.pause();
    }
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-8 bg-black/60 backdrop-blur-md animate-in fade-in duration-500">
      <div className="w-full max-w-sm bg-white rounded-[3rem] p-10 flex flex-col items-center space-y-8 shadow-2xl scale-in-95">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-[10px] font-black text-[#8B0000] uppercase tracking-[0.3em]">
        ...
        <div className="flex gap-4 w-full pt-4">
          ...
          <Button onClick={handleAccept} className="flex-1 h-20 rounded-3xl bg-[#8B0000] text-white font-black uppercase tracking-widest text-[10px] shadow-lg shadow-red-100 border-none active:scale-95">
